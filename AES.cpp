...
vector<int> AES::CreateWi(vector<vector<int>> finalkey, int idx){
    vector<int> wi(4);
    for(int x = 0; x < 4; x++)
        wi[x] = finalkey[x][idx];
...
void AES::AddRoundKey(vector<vector<int>> *user_input ,vector<vector<int>>  *new_key, int start){
    for(int i=0; i < 4; i++){
        for (int j = 0; j < 4; j++){
...
vector<int> AES::XorWi(vector<int> source, vector<int> second)
{
    for(int i=0; i < 4; i++)
        source[i] ^= second[i];
    return source;
...
vector<vector<int>> AES::KeyExpand(vector<vector<int>> finalkey){
...
vector<int> AES::MakeSubBytes(vector<int> user_input) {
...
int AES::GladiousMul(int a, int b)
{
    int p=0;
...
vector<vector<int>> AES::invs_sbox = {
...
void AES::InvUseSubBytes(vector<vector<int>> &user_input){
...
vector<int> AES::InvRotWord(vector<int> word)
{
    // Store the first value to move it to the bottom
    int first_val = word[0];
    // Shift the values up
...
void AES::InvShiftRows(vector<vector<int>> &user_input)
{
    for(int row_cnt=1; row_cnt < 4; row_cnt++)
    {
        for(int i=0; i < row_cnt; i++)
...
void AES::InvMixColumns(vector<vector<int>> &user_input) {
    for(int i=0; i < 4; i++) {
        vector<int> a = CreateWi(user_input, i);   // Get column
        int result[4];  // Doesn't exit the function. Thus no vector is needed
...
vector<vector<int>> AES::Decrypt(vector<vector<int>> &user_input,vector<vector<int>> &finalkey){
